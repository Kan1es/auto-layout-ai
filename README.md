...
Полный пользовательский сценарий через UI пока не готов к демонстрации без ограничений: между frontend и backend есть несовпадения контрактов, реальный DART и CVAT должны быть установлены отдельно, а endpoint автоматического импорта в CVAT отсутствует. Подробности зафиксированы в [`docs/kan-174-smoke-report.md`](docs/kan-174-smoke-report.md).
...
Этот сценарий является целевым. Перед демонстрацией нужно устранить блокеры из отчёта KAN-174 и убедиться, что DART и CVAT доступны локально.
...
Автоматический endpoint `/api/datasets/{dataset_id}/cvat/import` в текущей версии отсутствует, несмотря на наличие кнопки в UI.
...
- Полный UI-flow заблокирован текущими несовпадениями frontend/backend контрактов.

## Troubleshooting

### UI не переходит к статистике после загрузки ZIP

Текущий upload API возвращает идентификатор в `response.dataset.id`, а frontend ищет его в корне ответа. Это известная проблема KAN-174, предназначенная для интеграционной стабилизации.

### Representative next возвращает HTTP 500

В текущем backend используется `random.choice`, но модуль `random` не импортирован. Дефект зафиксирован smoke test KAN-174.
...
Автоматический endpoint ещё не реализован. Использовать ручной YOLO-экспорт и импорт по инструкции выше.
