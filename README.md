«Авторазметка» - Байзульдинов З. С., Кузьмичев А. А., Шипулин Л. М., Коростелев М.С., Журавлев Д.А. (Б-ИФСТ-11)
...
samples/              Небольшие локальные картинки для проверки DART
...
- Python 3.11+;
- зависимости из `requirements.txt`, включая Pillow (`PIL`) для обработки изображений.
...
Frontend уже раздаётся FastAPI-приложением: отдельный Node.js, npm или второй сервер не нужны.

В новом окне PowerShell перейдите в корень репозитория и выполните:

```powershell
Set-Location "<путь-к-папке-auto-layout-ai>"
...
# Создаёт samples/test.jpg для технической проверки DART.
.\.venv\Scripts\python.exe scripts/create_sample_image.py
```

Если в проекте уже есть `.venv`, повторно создавать его не нужно. Если PowerShell блокирует `Activate.ps1`, в том же окне выполните `Set-ExecutionPolicy -Scope Process Bypass`; команды выше не требуют активации окружения.

Запуск без DART (можно загружать датасеты и проходить этапы до preview):
...
Откройте `http://127.0.0.1:8000/`. Остановить сервер можно сочетанием `Ctrl+C`.
...
### 3. Установить PyTorch и GroundingDINO

Выберите **один** вариант ниже. CUDA подходит только для NVIDIA GPU. Перед CUDA-установкой проверьте, что драйвер видит видеокарту:
...
GroundingDINO собирает собственное CUDA-расширение. Поэтому нужны установленный NVIDIA Driver и CUDA Toolkit с `nvcc`; версия Toolkit должна совпадать с выбранной сборкой PyTorch. Проверка:
...
Выберите одну совместимую сборку PyTorch. Актуальные варианты и команды всегда есть в официальном селекторе: <https://pytorch.org/get-started/locally/>.

```powershell
Set-Location .\external\GroundingDINO

# Выполнить ТОЛЬКО ОДНУ строку, соответствующую установленному CUDA Toolkit.
python -m pip install torch torchvision --index-url https://download.pytorch.org/whl/cu118
# python -m pip install torch torchvision --index-url https://download.pytorch.org/whl/cu126
# python -m pip install torch torchvision --index-url https://download.pytorch.org/whl/cu128

# Указать компилятору путь к CUDA Toolkit только для текущего окна PowerShell.
$nvccPath = (Get-Command nvcc -ErrorAction Stop).Source
$env:CUDA_HOME = Split-Path (Split-Path $nvccPath -Parent) -Parent
```

После выбора CPU или CUDA завершите установку GroundingDINO:
...
Если после CUDA-установки команда показывает `CUDA available: False`, не запускайте DART в режиме GPU: проверьте `nvidia-smi`, версию CUDA Toolkit и выбранный индекс PyTorch. При ошибке `_C is not defined` удалите `external/GroundingDINO`, скачайте его заново и повторите установку после задания `CUDA_HOME`.
...
Для NVIDIA GPU замените последнюю строку на `--device cuda` после успешной проверки `CUDA available: True`.
...
## Запуск backend и frontend с DART
